...
/** When Gemini marks a detection as not a pothole / false positive — distinct from severity “low”. */
const GEMINI_NOT_POTHOLE_COLOR = '#10b981';
      ...
      {!expanded ? (
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 px-3 py-2">
          ...
          <div
            ...
            style={{ backgroundColor: GEMINI_NOT_POTHOLE_COLOR }}
            title="Gemini: not a pothole"
          />
        </div>
      ) : (
        <div className="px-4 py-3">
          ...
          <div className="space-y-2">
            ...
            <div className="mt-2 border-t border-white/20 pt-2">
              <div className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-white/80">Gemini</div>
              <div className="flex items-center space-x-2">
                <div className="h-3 w-3 rounded-full border border-white" style={{ backgroundColor: GEMINI_NOT_POTHOLE_COLOR }} />
                <span className="text-xs">Not a pothole / false alarm</span>
              </div>
            </div>
          </div>
        </div>
...
export const MapboxView = forwardRef<MapboxViewRef, MapboxViewProps>(({ potholes, onSelectPothole, selectedPotholeId = null }, ref) => {
  ...
  const [geminiCacheRev, setGeminiCacheRev] = useState(0);

  useEffect(() => {
    const bump = () => setGeminiCacheRev((n) => n + 1);
    window.addEventListener(GEMINI_CACHE_UPDATED_EVENT, bump);
    return () => window.removeEventListener(GEMINI_CACHE_UPDATED_EVENT, bump);
  }, []);
  ...
  useEffect(() => {
    ...
    potholes.forEach(pothole => {
      ...
      const color = notPotholeAi
        ? GEMINI_NOT_POTHOLE_COLOR
        : SEVERITY_COLORS[pothole.severity as keyof typeof SEVERITY_COLORS] || '#3b82f6';
      ...
      el.addEventListener('click', (e) => {
        ...
        const popupContent = `
          <div class="p-4 min-w-[200px] pr-10">
            ...
            ${
              notPotholeAi
                ? `<div class="mb-3 rounded-md px-2 py-1.5 text-xs font-semibold" style="background-color: ${GEMINI_NOT_POTHOLE_COLOR}22; color: #047857;">Gemini: not a pothole (likely false alarm)</div>`
                : ''
            }
            ...
            <div>
              ...
              <div class="flex items-center gap-2">
                ...
                <span class="font-semibold uppercase text-sm" style="color: ${color}">${notPotholeAi ? '—' : pothole.severity}</span>
              </div>
            </div>
          </div>
        `;
  ...
  }, [potholes, isMapLoaded, onSelectPothole, geminiCacheRev]);
