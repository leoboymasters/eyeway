...
import { shortImageDescriptionFromAnalysis } from '@/lib/geminiImageBlurb';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export const PotholeDetails = ({
  ...
  /** Matches `geminiBatchCache` stamp when the Supabase detail row is loaded. */
  const [detailRowStamp, setDetailRowStamp] = React.useState<string | null>(null);
  const [cachedGemini, setCachedGemini] = React.useState<RoadImageGeminiAnalysis | null>(null);
  const prevPotholeIdRef = React.useRef<string | null>(null);

  const reconcileGeminiCache = React.useCallback((id: string, stamp: string | null) => {
    if (!stamp) {
      setCachedGemini(null);
      return;
    }
    const e = getGeminiBatchEntry(id);
    setCachedGemini(e?.rowStamp === stamp ? e.analysis : null);
  }, []);

  React.useEffect(() => {
    if (!pothole?.id) return;
    const onUpd = () => reconcileGeminiCache(pothole.id, detailRowStamp);
    window.addEventListener(GEMINI_CACHE_UPDATED_EVENT, onUpd);
    return () => window.removeEventListener(GEMINI_CACHE_UPDATED_EVENT, onUpd);
  }, [pothole?.id, detailRowStamp, reconcileGeminiCache]);

  React.useEffect(() => {
    if (!pothole?.id) {
      prevPotholeIdRef.current = null;
      setPreviewUrl(null);
      setCropUrl(null);
    ...
    pothole?.id,
    pothole?.images?.[0],
    pothole?.frameImageUrl,
    pothole?.bboxXyxy,
    pothole?.frameWidth,
    pothole?.frameHeight,
    reconcileGeminiCache,
  ]);
  ...
  const geminiSaysNotPothole =
    cachedGemini != null &&
    (cachedGemini.assessment === 'unlikely_pothole' ||
      cachedGemini.review_filter === 'likely_false_positive');

  // Prefer full scene_summary from local Gemini cache so the panel is not stuck on an older/truncated DB blurb.
  const imageDescriptionText =
    cachedGemini?.scene_summary?.trim() ||
    pothole.description?.trim() ||
    (cachedGemini ? shortImageDescriptionFromAnalysis(cachedGemini) : '');

  const descriptionExtraContext =
    cachedGemini != null
      ? {
          surf: [cachedGemini.road_surface, cachedGemini.lighting_conditions]
            .filter(Boolean)
            .join(' · '),
          issues: cachedGemini.visible_issues.slice(0, 4).join(' · '),
          feats: cachedGemini.distinguishing_features.slice(0, 3).join(' · '),
        }
      : null;
  ...
  return (
    <Card
      className={cn(
        ...
        'animate-fade-in',
        geminiSaysNotPothole && 'border-l-4 border-emerald-500',
      )}
    >
      <div
      ...
      <CardContent className="space-y-2 px-0 pb-3 pt-2">
        ...
        {imageDescriptionText ? (
          <div className="rounded-lg border border-slate-200/80 bg-white p-2">
            <h4 className={cn('mb-1', lbl)}>Description</h4>
            <p className="whitespace-pre-wrap break-words text-[13px] leading-relaxed text-slate-800">
              {imageDescriptionText}
            </p>
            {descriptionExtraContext ? (
              <div className="mt-2 space-y-1.5 border-t border-slate-100 pt-2 text-[11px] leading-snug text-slate-600">
                {descriptionExtraContext.surf ? <p>{descriptionExtraContext.surf}</p> : null}
                {descriptionExtraContext.issues ? (
                  <p className="text-slate-500">{descriptionExtraContext.issues}</p>
                ) : null}
                {descriptionExtraContext.feats ? (
                  <p className="text-slate-500">{descriptionExtraContext.feats}</p>
                ) : null}
              </div>
            ) : null}
          </div>
        ...
        {/* Gemini (batch from map FAB) — shown when this row is cached for the current image/stamp */}
        {cachedGemini ? (
          <div className="rounded-lg border border-violet-200/70 bg-violet-50/25 p-2.5">
            <p className="text-sm font-semibold text-slate-900">Gemini analysis</p>
            <p className="mt-0.5 text-[10px] leading-snug text-slate-500">
              From the dashboard batch run (lower right). Re-run if you change images in Supabase.
            </p>
            <div className="mt-2 space-y-2 rounded-md border border-violet-100/90 bg-white/90 p-2 text-[12px] leading-snug text-slate-800">
              <div className="flex flex-wrap gap-1.5">
                <Badge
                  variant="outline"
                  className={cn(
                    'h-6 border px-2 text-[10px] font-medium',
                    cachedGemini.review_filter === 'keep' &&
                      'border-emerald-200/90 bg-emerald-50/90 text-emerald-900',
                    cachedGemini.review_filter === 'manual_review' &&
                      'border-amber-200/90 bg-amber-50/90 text-amber-900',
                    cachedGemini.review_filter === 'likely_false_positive' &&
                      'border-rose-200/90 bg-rose-50/90 text-rose-900',
                  )}
                >
                  {cachedGemini.review_filter === 'keep' && 'Filter: keep'}
                  {cachedGemini.review_filter === 'manual_review' && 'Filter: review'}
                  {cachedGemini.review_filter === 'likely_false_positive' &&
                    'Filter: likely false positive'}
                </Badge>
                <Badge variant="outline" className="h-6 border-slate-200/90 px-2 text-[10px] text-slate-800">
                  {cachedGemini.assessment === 'likely_pothole' && 'Assessment: likely pothole'}
                  {cachedGemini.assessment === 'unlikely_pothole' && 'Assessment: unlikely pothole'}
                  {cachedGemini.assessment === 'uncertain' && 'Assessment: uncertain'}
                </Badge>
                <span className="text-[10px] text-slate-500">
                  Confidence {(cachedGemini.confidence_0_to_1 * 100).toFixed(0)}%
                </span>
              </div>

              <div className="grid grid-cols-2 gap-2">
                <div>
                  <div className={lbl}>Road surface</div>
                  <p className="mt-0.5 text-[11px] text-slate-800">{cachedGemini.road_surface}</p>
                </div>
                <div>
                  <div className={lbl}>Lighting</div>
                  <p className="mt-0.5 text-[11px] text-slate-800">{cachedGemini.lighting_conditions}</p>
                </div>
              </div>

              {cachedGemini.visible_issues.length > 0 ? (
                <div>
                  <div className={lbl}>Visible issues</div>
                  <ul className="mt-0.5 list-inside list-disc space-y-0.5 text-[11px] text-slate-800">
                    {cachedGemini.visible_issues.map((t, i) => (
                      <li key={`vi-${i}-${t.slice(0, 40)}`}>{t}</li>
                    ))}
                  </ul>
                </div>
              ) : null}
              {cachedGemini.distinguishing_features.length > 0 ? (
                <div>
                  <div className={lbl}>Features</div>
                  <ul className="mt-0.5 list-inside list-disc space-y-0.5 text-[11px] text-slate-800">
                    {cachedGemini.distinguishing_features.map((t, i) => (
                      <li key={`df-${i}-${t.slice(0, 40)}`}>{t}</li>
                    ))}
                  </ul>
                </div>
              ) : null}
              {cachedGemini.caveats.length > 0 ? (
                <div>
                  <div className={lbl}>Caveats</div>
                  <ul className="mt-0.5 list-inside list-disc space-y-0.5 text-[11px] text-amber-900/90">
                    {cachedGemini.caveats.map((t, i) => (
                      <li key={`cv-${i}-${t.slice(0, 40)}`}>{t}</li>
                    ))}
                  </ul>
                </div>
              ) : null}
      ...
      <Dialog open={lightbox !== null} onOpenChange={(open) => !open && setLightbox(null)}>
        <DialogContent
          className={cn(
            'fixed inset-0 left-0 top-0 z-[60] flex h-[100dvh] max-h-[100dvh] w-screen max-w-none translate-x-0 translate-y-0 flex-col items-center justify-center gap-0 overflow-hidden border-0 bg-black/95 p-0 shadow-none sm:rounded-none',
            '[&>button]:right-[max(0.75rem,env(safe-area-inset-right))] [&>button]:top-[max(0.75rem,env(safe-area-inset-top))] [&>button]:h-10 [&>button]:w-10 [&>button]:rounded-full [&>button]:bg-black/50 [&>button]:text-white [&>button]:opacity-90 [&>button]:hover:bg-black/70 [&>button]:hover:opacity-100'
          )}
        >
          ...
          {lightbox ? (
            <div className="flex h-full w-full min-h-0 min-w-0 items-center justify-center px-0 py-0">
              {lightbox.detection ? (
                <ImageWithBboxOverlay
                  src={lightbox.src}
                  alt={lightbox.label}
                  detection={lightbox.detection}
                  imgClassName="max-h-[100dvh] max-w-[100vw] h-auto w-auto object-contain"
                />
              ) : (
                <img
                  src={lightbox.src}
